...
def _bruteforce_nearest(shape, seeds, anisotropy):
    ...
    Tie-breaks by choosing the larger seed index to match expand_labels_nd 1D.
    """
...
def test_feature_transform_nd_matches_bruteforce_2d():
    shape = (7, 9)
    ...
    feats = edt.feature_transform_nd(arr, anisotropy=anis, parallel=1)
...
def test_feature_transform_nd_return_distances_matches_edtsq_nd():
    shape = (6, 6, 4)
    arr = np.zeros(shape, dtype=np.uint32)
    arr[1, 1, 1] = 5
    arr[4, 2, 3] = 7
    ...
    feats, dist = edt.feature_transform_nd(
...
def test_expand_labels_nd_return_features_consistent_with_feature_transform():
    shape = (5, 8)
    arr = np.zeros(shape, dtype=np.uint32)
    arr[0, 0] = 1
    arr[4, 7] = 2
    ...
    labels, feats = edt.expand_labels_nd(
        arr, anisotropy=anis, parallel=1, return_features=True
    )
    ft = edt.feature_transform_nd(arr, anisotropy=anis, parallel=1)
    np.testing.assert_array_equal(feats, ft)
    np.testing.assert_array_equal(labels, arr.ravel()[feats].reshape(shape))


def test_feature_transform_nd_features_dtype_options():
    shape = (4, 4)
    arr = np.zeros(shape, dtype=np.uint32)
    arr[0, 0] = 1
    arr[3, 3] = 2

    feats_u32 = edt.feature_transform_nd(arr, features_dtype="uint32", parallel=1)
    feats_up = edt.feature_transform_nd(arr, features_dtype="uintp", parallel=1)
    assert feats_u32.dtype == np.uint32
    assert feats_up.dtype == np.uintp


def test_feature_transform_nd_anisotropy_length_mismatch_raises():
    arr = np.zeros((4, 4), dtype=np.uint32)
    with np.testing.assert_raises(ValueError):
        edt.feature_transform_nd(arr, anisotropy=(1.0, 2.0, 3.0))


def test_feature_transform_nd_1d_matches_bruteforce():
